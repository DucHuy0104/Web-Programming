...
function syncCartWithDatabase($pdo, $user_id) {
    ...
    // Merge với giỏ hàng trong session (ưu tiên session)
    ...
    $pdo->prepare("DELETE FROM user_carts WHERE user_id = ?")->execute([$user_id]);
    if (!empty($_SESSION['cart'])) {
        $insert_values = [];
        foreach ($_SESSION['cart'] as $item) {
            $insert_values[] = "({$user_id}, {$item['product_id']}, '{$item['size']}', {$item['quantity']})";
        }
        $query = "INSERT INTO user_carts (user_id, product_id, size, quantity) VALUES ".implode(',', $insert_values);
        $pdo->exec($query);
    }
}
